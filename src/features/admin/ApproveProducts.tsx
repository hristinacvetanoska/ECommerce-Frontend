...
export default function ApproveProducts() {
  ...
  useEffect(() => {
    {
      user &&
        agent.NewProduct.list("")
          .then((newProducts) => setNewProducts(newProducts))
          .catch((error) => console.log(error))
          .finally(() => setLoading(false));
    }
  }, []);
  function handleApproveProduct(response: boolean, id: number) {
    setLoading(true);
    setTarget(id);
    agent.Admin.approveProducts(response, id)
        ...
        <TableBody>
          {newProducts?.map((newProducts) => (
            ...
            >
              ...
              <TableCell align="right">
                <Button
                  onClick={() => handleApproveProduct(true, newProducts.id)}
                  startIcon={<CheckIcon />}
                />
                <Button
                  startIcon={<ClearIcon />}
                  color="error"
                  onClick={() => handleApproveProduct(false, newProducts.id)}
                />
              </TableCell>
            </TableRow>
          ))}
